...
export const TheaterDashboard: React.FC = () => {
    ...
    if (loading) return <div className="p-10 text-center animate-pulse text-red-500 font-black italic">Preparando o Palco...</div>;
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700 pb-20">
            ...
            <header className="relative p-12 rounded-[48px] overflow-hidden group">
                ...
                <div className="absolute inset-0 opacity-20 bg-[url('https://images.unsplash.com/photo-1503095396549-80705a61066d?q=80&w=2000&auto=format&fit=crop')] bg-cover bg-center z-0 group-hover:scale-105 transition-transform duration-1000"></div>
                
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-end gap-8">
                    <div>
                        <span className="text-red-500 font-black text-[10px] uppercase tracking-[0.4em] mb-4 block">Stage Control Center</span>
                        <h1 className="text-6xl font-black text-white tracking-tighter italic">O Fantasma da Ópera</h1>
                        <p className="text-slate-400 font-medium mt-4 max-w-xl leading-relaxed text-lg">
                            Próxima sessão em <strong className="text-white">2h 15min</strong>. Status técnico: <span className="text-green-400 font-bold uppercase tracking-widest text-xs">Ready for Curtain Call</span>
                        </p>
                    </div>
                    <div className="flex gap-4">
                        <Button className="bg-red-600 hover:bg-red-700 text-white px-8 py-7 rounded-[32px] font-black italic shadow-2xl shadow-red-600/20 flex items-center gap-3">
                            <Play size={20} fill="currentColor" /> Abrir Portas
                        </Button>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 px-2">
                {[
                    { label: "Ingressos Vendidos", val: stats.totalTickets, icon: <Ticket />, color: "text-red-400", bg: "bg-red-500/10" },
                    { label: "Receita Total", val: `R$ ${stats.revenue.toLocaleString()}`, icon: <TrendingUp />, color: "text-emerald-400", bg: "bg-emerald-500/10" },
                    { label: "Ocupação Média", val: `${stats.occupancy}%`, icon: <Users />, color: "text-blue-400", bg: "bg-blue-500/10" },
                    { label: "Sessões Ativas", val: stats.activeSessions, icon: <Clapperboard />, color: "text-purple-400", bg: "bg-purple-500/10" },
                ].map((item, idx) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 premium-glass p-10 rounded-[48px] border-white/5">
                    <div className="flex items-center justify-between mb-10">
                        <div>
                            <h3 className="text-2xl font-black text-white italic">Mapa de Assentos</h3>
                            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Sessão: Hoje, 20h00</p>
                        </div>
                        <div className="flex gap-4">
                             <div className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                <div className="w-3 h-3 bg-red-500 rounded-sm" /> Ocupado
                             </div>
                             <div className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                <div className="w-3 h-3 bg-white/10 rounded-sm border border-white/20" /> Livre
                             </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full h-8 bg-gradient-to-b from-slate-800 to-transparent rounded-full mb-12 flex items-center justify-center">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-[1em] ml-[1em]">PALCO</span>
                    </div>
                    ...
                    <div className="mt-12 pt-8 border-t border-white/5 flex justify-between items-center">
                        <div className="flex gap-8">
                            <div>
                                <span className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Disponíveis</span>
                                <span className="text-xl font-black text-white">18</span>
                            </div>
                            <div>
                                <span className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Reservados</span>
                                <span className="text-xl font-black text-white">42</span>
                            </div>
                        </div>
                        <Button variant="secondary" className="text-[10px] px-8 rounded-2xl">Gerenciar Lotes</Button>
                    </div>
                </div>
                ...
                <div className="space-y-8">
                    <div className="premium-glass p-8 rounded-[48px] border-red-500/20 bg-red-500/5">
                        <h3 className="text-lg font-black text-white italic mb-6 flex items-center gap-3">
                            <Mic2 className="text-red-500" /> Backstage Live
                        </h3>
                        <div className="space-y-6">
                            {[
                                { msg: "Iluminação: Canal 4 ok", time: "10m ago", status: "ok" },
                                { msg: "Aviso: Ator Principal no local", time: "25m ago", status: "info" },
                                { msg: "Urgente: Microfone de lapela 2 falhando", time: "40m ago", status: "warn" },
                            ].map((feed, i) => (
                    ...
                    <div className="premium-glass p-8 rounded-[48px] border-white/5">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-lg font-black text-white italic">Elenco & Crew</h3>
                            <UserPlus size={18} className="text-slate-500 cursor-pointer hover:text-white" />
                        </div>
                        <div className="space-y-4">
                            {[
                                { name: "Erik Destler", role: "Protagonista", img: "E" },
                                { name: "Christine Daaé", role: "Soprano", img: "C" },
                                { name: "Raoul de Chagny", role: "Tenor", img: "R" },
                                { name: "Madame Giry", role: "Coreógrafa", img: "M" },
                            ].map((person, i) => (
