...
import { AlertTriangle, Building, Accessibility, CheckCircle2, Info, MapPin, Search, ArrowUpRight, HandMetal, Volume2, Activity, Scale } from "lucide-react";
import { 
    Card, 
    Badge, 
    Button, 
...
type SecretaryDashboardResponse = {
    equipmentAccessibility?: EquipmentAccessibility[];
};
...
export const MunicipalAccessibilityGaps: React.FC = () => {
    ...
    const [data, setData] = useState<SecretaryDashboardResponse | null>(null);
    const [matrix, setMatrix] = useState<LegalComplianceItem[]>([]);
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const [dashboardRes, matrixRes] = await Promise.all([
                api.get<SecretaryDashboardResponse>("/secretary/dashboard", { params: { tenantId, consolidated: true } }),
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
            ...
            <p className="text-slate-500 font-black animate-pulse uppercase tracking-widest text-[10px]">{t("municipal.accessibility.loading", "Mapeando Vazios de Acessibilidade...")}</p>
        </div>
    );

    const equipments = data?.equipmentAccessibility || [];
    const totalCount = equipments.length;
    const okCount = equipments.filter((e) => e.hasAccessibility).length;
        ...
        <AnimateIn className="space-y-12 pb-32">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div className="space-y-3">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-white tracking-tighter leading-none">
                            {t("municipal.accessibility.title_part1", "Auditoria de")} <span className="text-emerald-500">{t("municipal.accessibility.title_part2", "Acessibilidade")}</span>
                        </h1>
                    </div>
                    <p className="text-slate-500 font-medium text-lg">{t("municipal.accessibility.subtitle", "Mapeamento de vazios críticos e conformidade com a LBI.")}</p>
                </div>
                ...
                <div className="flex gap-3">
                    <Button variant="glass" className="h-14 px-6 rounded-2xl border-white/5 text-slate-400 font-black uppercase text-[10px] tracking-widest">
                        {t("municipal.accessibility.export", "Exportar Laudo Técnico")}
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="p-8 bg-white/[0.02] border-white/5 rounded-[40px] group hover:bg-white/[0.04] transition-all">
                    ...
                    <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-2">{t("municipal.accessibility.monitored_units", "Unidades Culturais Monitoradas")}</div>
                </Card>
                ...
                <Card className="p-8 bg-white/[0.02] border-white/5 rounded-[40px] group hover:bg-white/[0.04] transition-all">
                    ...
                    <div className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mt-2">{t("municipal.accessibility.compliant_units", "Unidades em Conformidade LBI")}</div>
                </Card>
                ...
                <Card className="p-8 bg-white/[0.02] border-white/5 rounded-[40px] group hover:bg-white/[0.04] transition-all relative overflow-hidden">
                    ...
                    <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-2">{t("municipal.accessibility.assistive_tech_coverage", "Cobertura de Tecnologia Assistiva")}</div>
            ...
            <Card className="p-0 bg-white/[0.02] border-white/5 rounded-[48px] overflow-hidden">
                <div className="p-10 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 bg-white/[0.01]">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-white tracking-tight italic">{t("municipal.accessibility.status_per_equipment", "Status por Equipamento")}</h3>
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-600">{t("municipal.accessibility.mapping_gaps", "Mapeamento de vazios e recursos")}</p>
                        </div>
                    </div>
                    <div className="relative w-full md:w-80 group">
                        ...
                        <input 
                            type="text" 
                            placeholder={t("municipal.accessibility.search_unit", "Buscar unidade...")} 
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead className="bg-white/[0.02] text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">
                            <tr>
                                <th className="px-10 py-6">{t("municipal.accessibility.table_col1", "Unidade Cultural")}</th>
                                <th className="px-10 py-6">{t("municipal.accessibility.table_col2", "Tipologia")}</th>
                                <th className="px-10 py-6 text-center">{t("municipal.accessibility.table_col3", "Tecnologias Assistivas")}</th>
                                <th className="px-10 py-6">{t("municipal.accessibility.table_col4", "Status Final")}</th>
                            ...
                            {equipments.map((eq) => (
                                <tr key={eq.id} className="hover:bg-white/[0.02] transition-all group">
                                    <td className="px-10 py-8">
                                        ...
                                        <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest mt-1">{t("municipal.accessibility.ref", "Ref:")} {eq.id.slice(0, 8)}</div>
                                    </td>
                                    <td className="px-10 py-8">
                                    ...
                                    <td className="px-10 py-8">
                                       <div className="flex justify-center gap-3">
                                           <div className={`p-2 rounded-lg border transition-all ${eq.hasLibras ? 'bg-indigo-500/20 border-indigo-500 text-indigo-400' : 'bg-white/5 border-white/5 text-slate-700'}`} title={t("municipal.accessibility.libras", "Libras")}>
                                               <HandMetal size={16} />
                                           </div>
                                           <div className={`p-2 rounded-lg border transition-all ${eq.hasAudioDescription ? 'bg-indigo-500/20 border-indigo-500 text-indigo-400' : 'bg-white/5 border-white/5 text-slate-700'}`} title={t("municipal.accessibility.audio_description", "Audiodescrição")}>
                                               <Volume2 size={16} />
                                           </div>
                                           <div className={`p-2 rounded-lg border transition-all ${eq.hasRamps ? 'bg-indigo-500/20 border-indigo-500 text-indigo-400' : 'bg-white/5 border-white/5 text-slate-700'}`} title={t("municipal.accessibility.physical_accessibility", "Acessibilidade Física")}>
                                    ...
                                    <td className="px-10 py-8">
                                        {eq.hasAccessibility ? (
                                            <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 uppercase text-[9px] font-black tracking-widest px-4 py-1.5">
                                                {t("municipal.accessibility.compliance_ok", "Conformidade OK")}
                                            </Badge>
                                        ) : (
                                            <Badge className="bg-rose-500/10 text-rose-500 border-rose-500/20 uppercase text-[9px] font-black tracking-widest px-4 py-1.5 flex items-center gap-2">
                                                <AlertTriangle size={12} /> {t("municipal.accessibility.critical_gap", "Vazio Crítico")}
                                            </Badge>
                                        )}
                                    </td>
            ...
            <div className="space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-black text-white tracking-tighter italic">{t("municipal.accessibility.matrix_title", "Matriz de Conformidade Legal (LBI)")}</h2>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-600">{t("municipal.accessibility.matrix_subtitle", "Requisitos obrigatórios para gestão cultural municipal")}</p>
                    </div>
                </div>
                                ...
                                <div className="relative z-10 flex flex-col h-full space-y-8">
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-2">
                                            <Badge variant="glass" className="bg-emerald-500/10 text-emerald-400 border-none text-[9px] font-black uppercase tracking-widest">
                                                {item.law || t("municipal.accessibility.lbi_law", "Lei Brasileira de Inclusão")}
                                            </Badge>
                                    ...
                                    <div className="space-y-6 flex-1">
                                        <div className="p-6 bg-white/[0.01] border border-white/5 rounded-3xl space-y-2">
                                            <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">{t("municipal.accessibility.evidence_identified", "Evidência Identificada")}</p>
                                        ...
                                        <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-500 bg-white/5 p-4 rounded-2xl border border-white/5">
                                            ...
                                            {item.howWeComply || t("municipal.accessibility.compliance_strategy", "Estratégia de conformidade em implementação.")}
                                        </div>
                                    </div>
                                </div>
