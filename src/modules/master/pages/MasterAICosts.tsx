  ...
  const { t } = useTranslation();
    ...
    const fetchReport = useCallback(async () => {
        try {
            ...
            console.error(error);
            toast.error("Erro ao carregar relat�rio de IA");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    return (
        <div style={{ display: "grid", gap: "2rem" }}>
            <div>
                ...
                <p style={{ color: "#64748b", fontSize: "0.85rem", marginTop: "0.25rem" }}>Relat�rio global de uso e custos � {periodLabel}</p>
            </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-black/40 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                            <tr>
                                ...
                                <th className="px-6 py-4">{t("master.aicosts.anlises", `An�lises`)}</th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            ...
                            {report.topConsumers.length === 0 && (
                                <tr><td colSpan={5} className="px-6 py-10 text-center text-gray-500 italic">{t("master.aicosts.nenhumConsumoRegistradoNestePerodo", `Nenhum consumo registrado neste per�odo.`)}</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            {report.nearLimitTenants.length > 0 && (
                <div>
                    <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                        <AlertTriangle size={20} className="text-amber-500" /> Tenants Pr�ximos do Limite (80%+)
                    </h2>
                    <div style={{ display: "grid", gap: "0.75rem" }}>
                        {report.nearLimitTenants.map(item => (
                            <div key={item.tenantId} className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] shadow-[var(--shadow-surface)] rounded-[var(--radius-lg)] p-6 transition-colors" style={{ padding: "1.25rem", borderColor: "rgba(212,175,55,0.2)" }}>
                                ...
                                <p className="text-gray-500 text-xs mt-2">{item.current} / {item.limit} an�lises</p>
                            </div>
                        ))}
                    </div>
                </div>
