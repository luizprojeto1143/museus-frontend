    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 font-serif">Notificações Push</h1>
                    <p className="text-slate-500">Envie mensagens em tempo real para todos os usuários do seu museu/instituição.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <form onSubmit={handleSend} className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">Título da Notificação</label>
                            <input
                                type="text"
                                value={title}
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">Mensagem (Corpo)</label>
                            <textarea
                                value={body}
                                onChange={e => setBody(e.target.value)}
                                className="w-full border border-slate-200 rounded-xl px-4 py-2 focus:border-amber-500 transition-colors outline-none resize-none"
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">Link de Destino (URL)</label>
                            <div className="flex gap-2">
                                <span className="bg-slate-100 px-3 py-2 rounded-xl text-slate-400 text-sm flex items-center">/</span>
                                <input
                                    type="text"
                                    value={url.startsWith('/') ? url.slice(1) : url}
                                    onChange={e => setUrl('/' + e.target.value)}
                                    className="flex-1 border border-slate-200 rounded-xl px-4 py-2 focus:border-amber-500 transition-colors outline-none"
                ...
                <div className="space-y-6">
                    <div className="bg-amber-50 border border-amber-100 rounded-2xl p-6">
                        ...
                        <ul className="text-amber-700 text-sm space-y-2 list-disc pl-4">
