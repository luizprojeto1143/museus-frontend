...
export const getVisitorLinks = (t: TFunction, term: Terminology, isCityMode: boolean): NavLink[] => {
  ...
  return [
    ...
    { to: base ? `${base}/obras` : "/obras", label: term.works, icon: isCityMode ? "🏛️" : "🎨", feature: "featureWorks" },
    { to: base ? `${base}/trilhas` : "/trilhas", label: term.trails, icon: "🗺️", feature: "featureTrails" },
    { to: ctx ? buildMuseumMapUrl(ctx.citySlug, ctx.equipmentSlug) : "/mapa", label: _t("visitor.sidebar.map", "Mapa"), icon: "📍", feature: null },
    { to: "/agenda", label: "Agenda Cultural", icon: "🎟️", feature: null },
    { to: "/meus-ingressos", label: "Meus Ingressos", icon: "🎫", feature: null },
    { to: base ? `${base}/eventos` : "/eventos", label: _t("visitor.sidebar.events"), icon: "📅", feature: "featureEvents" },
