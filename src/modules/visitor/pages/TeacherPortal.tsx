    ...
    return (
        <div style={{ padding: '1.5rem', maxWidth: '700px', margin: '0 auto' }}>
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
                <GraduationCap size={36} style={{ color: 'var(--accent-primary)', margin: '0 auto 0.5rem' }} />
            ...
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center', marginBottom: '1.5rem' }}>
                <button onClick={() => setTab('visits')} style={{ padding: '0.5rem 1.5rem', borderRadius: '2rem', border: 'none', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', background: tab === 'visits' ? 'var(--accent-primary)' : 'rgba(255,255,255,0.05)', color: tab === 'visits' ? '#1a1108' : '#888' }}>
                    <Calendar size={14} style={{ display: 'inline', marginBottom: '-2px', marginRight: '0.35rem' }} />Visitas
                </button>
                <button onClick={() => setTab('resources')} style={{ padding: '0.5rem 1.5rem', borderRadius: '2rem', border: 'none', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', background: tab === 'resources' ? 'var(--accent-primary)' : 'rgba(255,255,255,0.05)', color: tab === 'resources' ? '#1a1108' : '#888' }}>
            ...
            {tab === 'visits' && (
                <div>
                    <button onClick={() => setShowForm(!showForm)} style={{ width: '100%', background: 'rgba(212,175,55,0.1)', border: '1px solid rgba(212,175,55,0.2)', borderRadius: '1rem', padding: '1rem', color: 'var(--accent-primary)', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                    ...
                    {showForm && (
                        <div style={{ background: 'rgba(30,32,38,0.9)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '1.25rem', padding: '1.5rem', display: 'grid', gap: '0.75rem', marginBottom: '1rem' }}>
                            ...
                            <button onClick={onSubmitVisit} style={{ width: '100%', background: 'linear-gradient(135deg, var(--accent-primary), #b8941e)', color: '#1a1108', border: 'none', padding: '0.75rem', borderRadius: '1rem', fontWeight: 800, cursor: 'pointer' }}>Agendar</button>
                        </div>
                    )}
                        ...
                        {visits.map((v: any) => (
                            <div key={v.id} style={{ background: 'rgba(30,32,38,0.9)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '1rem', padding: '1rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <span style={{ fontSize: '0.7rem', fontWeight: 700, padding: '0.15rem 0.5rem', borderRadius: '0.25rem', background: v.status === 'CONFIRMED' ? 'rgba(34,197,94,0.1)' : 'rgba(212,175,55,0.1)', color: v.status === 'CONFIRMED' ? '#22c55e' : 'var(--accent-primary)' }}>{v.status || 'PENDENTE'}</span>
                                </div>
