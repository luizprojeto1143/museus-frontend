...
import { persistMuseumFromEquipment, museumEquipmentPath } from "@/config/golive";
...
export const SelectMuseum: React.FC = () => {
  ...
  const nearestEquipamentos = useMemo(() => {
    ...
    return [...filteredAndSortedEquipamentos]
      .filter(e => e.distance !== undefined && e.distance < 5)
      .slice(0, 3);
  ...
  const goToEquipmentHub = (equip: Equipamento) => {
    const ctx = persistMuseumFromEquipment(equip);
    const path = museumEquipmentPath(equip);
    if (!ctx || !path) {
      setErrorMsg("Selecione um museu com cidade e identificador válidos para entrar no Hub.");
      return false;
    }
    return path;
  };
  ...
  const handleSelect = async (equip: Equipamento) => {
    if (isAuthenticated && !isGuest) {
      try {
        const path = goToEquipmentHub(equip);
        if (!path) return;
        ...
        <div className="pulse-top-actions flex items-center gap-4">
          <Button
            ...
            onClick={() => {
              ...
              const path = goToEquipmentHub(museum);
              if (!path) return;
      ...
      <header className="pulse-hero py-24 px-8 text-center relative z-10">
        <AnimateIn variant="fadeUp">
          <div className="pulse-hero-content max-w-4xl mx-auto">
            ...
            <p className="pulse-subtitle">{t("visitor.selectMuseum.radarSubtitle")}</p>
            <div className="pulse-search-wrapper max-w-2xl mx-auto space-y-8">
              <div className="relative group transition-all duration-500">
                ...
                <div className="pulse-search-container">
                  ...
                  <input type="text" placeholder={t("visitor.selectMuseum.searchPlaceholder")} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pulse-search-input" />
                </div>
              </div>
              <div className="pulse-filters flex flex-wrap justify-center gap-3">
                {[
                  ...
                  <Button key={f.id} variant={activeFilter === f.id ? "primary" : "glass"} size="sm" onClick={() => setActiveFilter(f.id)} className="rounded-full px-6 py-2 h-auto" leftIcon={f.icon}>{f.label}</Button>
                ))}
              </div>
            </div>
          </div>
        ...
        {!searchTerm && activeFilter !== "EVENTOS" && events.length > 0 && (
          <AnimateIn variant="fadeUp" className="mt-16 w-full max-w-7xl mx-auto overflow-hidden">
            <div className="flex items-center justify-between mb-8 px-4">
              <h2 className="text-xl font-bold flex items-center gap-3"><Sparkles className="text-gold-400" size={20} /> Agenda Cultural</h2>
              <Button variant="ghost" size="sm" onClick={() => setActiveFilter("EVENTOS")} className="text-gold-400 hover:text-gold-300">Ver Tudo</Button>
            </div>
            <div className="flex gap-6 overflow-x-auto pb-8 px-4 snap-x no-scrollbar">
              {events.slice(0, 8).map(event => (
                <motion.div key={event.id} whileHover={{ y: -10 }} className="flex-shrink-0 w-80 snap-start">
                  <Card className="h-full border-white/5 bg-white/5 backdrop-blur-md overflow-hidden cursor-pointer group" onClick={() => navigate(`/events`)}>
                    <div className="relative h-44">
                      <img src={event.coverImage || "/placeholder-image.svg"} alt={event.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                      <div className="absolute top-4 left-4 bg-gold-400 text-black text-[10px] font-black px-2 py-1 rounded uppercase">{event.category}</div>
                    </div>
                    <div className="p-5">
                      <h4 className="font-bold text-white line-clamp-1 mb-2 group-hover:text-gold-400 transition-colors">{event.title}</h4>
          ...
          <AnimateIn variant="fadeUp">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {nearestEquipamentos.map(e => (
                <Card animated className="p-6 cursor-pointer border-[var(--accent-primary)]/10 group bg-[var(--accent-primary)]/5 backdrop-blur-md" onClick={() => handleSelect(e)}>
                  <div className="flex items-center gap-6">
                    <div className="w-14 h-14 rounded-2xl bg-[var(--accent-primary)]/10 flex items-center justify-center font-mono font-bold text-[var(--accent-primary)] border border-[var(--accent-primary)]/20 group-hover:scale-110 transition-transform">{formatDistance(e.distance)}</div>
                    <div>
          ...
          <motion.div variants={staggerContainer()} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {events.map(event => (
              <motion.div key={event.id} variants={staggerItem}>
                <Card animated glow className="h-full flex flex-col group cursor-pointer overflow-hidden border-white/5" onClick={() => navigate(`/events`)}>
                  <div className="relative h-60 overflow-hidden">
                    <img src={event.coverImage || "/placeholder-image.svg"} alt={event.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    <Badge className="absolute top-4 left-4 bg-black/60 shadow-xl" variant="glass"><Calendar size={12} className="mr-2" />{event.category}</Badge>
                  </div>
          ...
          <div className="flex flex-col gap-4">
            {!searchTerm && (selectedEstado || selectedCidade) && (
              <div className="flex items-center gap-4 mb-4">
                 <Button variant="ghost" onClick={() => { if (selectedCidade) setSelectedCidade(null); else if (selectedEstado) setSelectedEstado(null); }} leftIcon={<ArrowRight className="rotate-180" size={16} />}>{selectedCidade ? "Voltar para Cidades" : "Voltar para Estados"}</Button>
                 <span className="text-[var(--fg-tertiary)] text-sm font-bold">{selectedEstado} {selectedCidade ? ` > ${selectedCidade}` : ""}</span>
              </div>
            )}
            <motion.div variants={staggerContainer()} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {searchTerm ? (
                filteredAndSortedEquipamentos.map(equip => (
                  <motion.div key={equip.id} variants={staggerItem}>
                    <Card animated glow className="h-full flex flex-col group cursor-pointer overflow-hidden border-white/5" onClick={() => handleSelect(equip)}>
                      <div className="relative h-60 overflow-hidden">
                        <img src={equip.fotoCapaUrl || "/placeholder-image.svg"} alt={equip.nome} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 group-hover:rotate-1" />
                        <Badge className="absolute top-4 left-4 bg-black/60 shadow-xl" variant="glass"><Theater size={12} className="mr-2" />{equip.tipo}</Badge>
                      </div>
                      <div className="p-8 flex-1 flex flex-col">
                ...
                estadosDisponiveis.map(estado => (
                   <motion.div key={estado} variants={staggerItem}>
                     <Card animated glow className="p-10 cursor-pointer text-center group border-white/5 bg-white/5 backdrop-blur-md" onClick={() => setSelectedEstado(estado)}>
                       ...
                       <p className="text-[var(--fg-tertiary)] mt-4 font-medium uppercase tracking-widest text-xs">{equipamentos.filter(e => (e.estado || "MG") === estado).length} Equipamentos</p>
                     </Card>
                   </motion.div>
                ))
              ) : !selectedCidade ? (
                cidadesDoEstado.map(cidade => (
                   <motion.div key={cidade} variants={staggerItem}>
                     <Card animated glow className="p-10 cursor-pointer text-center group border-white/5 bg-white/5 backdrop-blur-md" onClick={() => setSelectedCidade(cidade || null)}>
                       ...
                       <p className="text-[var(--fg-tertiary)] mt-4 font-medium uppercase tracking-widest text-xs">{equipamentos.filter(e => e.cidade === cidade && (e.estado || "MG") === selectedEstado).length} Equipamentos</p>
                     </Card>
                   </motion.div>
                ))
              ) : (
      ...
      {selectedLandmark && (
        <div className="landmark-modal-overlay fixed inset-0 z-[1000] bg-black/95 backdrop-blur-lg p-4 flex items-center justify-center animate-in fade-in duration-300" onClick={() => setSelectedLandmark(null)}>
          <Card className="max-w-4xl w-full max-h-[90vh] overflow-hidden relative border-white/10 shadow-2xl animate-in slide-in-from-bottom-8 duration-500" onClick={e => e.stopPropagation()}>
            <Button variant="glass" size="sm" className="absolute top-6 right-6 z-20 rounded-full w-12 h-12 p-0" onClick={() => setSelectedLandmark(null)}><X size={24} /></Button>
            ...
            <div className="p-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-10">
                <div className="space-y-4">
                  <div className="flex items-center gap-4 text-[var(--fg-secondary)]"><MapPin size={20} className="text-[var(--accent-primary)]" /><span className="font-medium">{selectedLandmark.endereco}</span></div>
                  <div className="flex items-center gap-4 text-[var(--fg-secondary)]"><Clock size={20} className="text-[var(--accent-primary)]" /><span className="font-medium">{selectedLandmark.horarios?.seg || t("visitor.selectMuseum.modal.visitationFree")}</span></div>
                </div>
                <div className="bg-[var(--bg-surface-hover)] p-6 rounded-2xl border border-[var(--border-subtle)]">
                  ...
                  <p className="text-[var(--fg-secondary)] text-sm leading-relaxed italic">{selectedLandmark.missao || selectedLandmark.descricao || t("visitor.selectMuseum.modal.historyFallback")}</p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-4">
                <Button className="flex-1 h-14 text-lg font-bold" onClick={() => {
                  const path = goToEquipmentHub(selectedLandmark);
                  if (!path) return;
