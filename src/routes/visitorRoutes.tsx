...
import { Role, useAuth } from "../modules/auth/AuthContext";
import { hasSelectedMuseum, readMuseumCtx } from "../config/golive";
...
const RequireMuseum: React.FC<{ children: React.ReactElement }> = ({ children }) => {
    ...
    if (!hasSelectedMuseum(tenantId)) {
        return <Navigate to="/select-museum" replace />;
    }
    return children;
};

const RedirectMuseumWorks: React.FC = () => {
    const ctx = readMuseumCtx();
    if (ctx?.citySlug && ctx?.equipmentSlug && ctx.citySlug !== "undefined" && ctx.equipmentSlug !== "undefined") {
        return <Navigate to={`${buildEquipmentUrl(ctx.citySlug, ctx.equipmentSlug)}/obras`} replace />;
    }
    return <Navigate to="/select-museum" replace />;
};
...
export function visitorRoutes(RequireRole: React.FC<RequireRoleProps>) {
    return (
        <>
            ...
            <Route path="/avatar" element={<Navigate to="/rpg" replace />} />
            <Route path="/batalha" element={<Navigate to="/rpg" replace />} />
            <Route path="/camera" element={<Navigate to="/scanner/ai" replace />} />
            ...
            <Route path="/badges" element={<Navigate to="/cracha/rastreio" replace />} />
